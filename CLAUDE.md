...
- **烧录脚本选择**（2026-09-22，用户提出"在 toml 里选芯片而不是写 cmm 路径"）：
  ...
  - 除 `CPU=` 外不推导任何脚本参数（2026-09-24，用户要求"需要配置的内容越少越好"，并且库里的脚本和官方脚本用法必须一样）。`DUALPORT=` 交给脚本默认值：它指的是 Flash 算法的双端口下载，和 `target.dual_port`（`SYStem.Option.DUALPORT`，运行时访问内存）不是一回事。JTAG 时钟由 tracebridge 在脚本之后用 `target.jtag_clock` 设置，不作为脚本参数。init 模板里 `args = []`。曾经加过自动传 `JTAG_CLOCK=` 的规则（a73b6dc），随后撤掉：这个参数只有我们自己的旧脚本才有。
  - SR6P6 脚本：FAE 于 2026-09 给了官方版（rev 17238，装在 `<sys>/demo/arm/flash/sr6p6.cmm`）。它不关 SWT_SYS_0 看门狗，所以 `~/.config/tracebridge/flash/sr6p6.cmm` 是在 rev 17238 上只加了 DisableWatchdog/WatchdogDisabled 的版本，参数和官方完全一样（PREPAREONLY、DUALPORT=），保留 CRLF，这样和官方 diff 只有新增行。旧的修改版（基于 rev 13225，带 ELF=、JTAG_CLOCK=）备份在 `~/.config/tracebridge/sr6p6.cmm.rev13225-modified.bak`。许可证是 "TRACE32 only"，**不得提交进仓库**。
